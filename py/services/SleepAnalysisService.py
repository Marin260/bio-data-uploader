...
class SleepAnalysisService:
    ...
    def zip_dir(self, directory, file_name):
        zip_file = os.path.join(self.SAVE_DIR, f'{file_name}.zip')
    ...
    def get_mean_and_sem_by_hour(self, df):
    ...
    def find_amount_of_sleep(self, df):
        np_data = df.to_numpy()
    ...
    def run(
        ...
        df = self.preproc_dataframe(start_date_time, end_date_time)
        ...
        for time_start_end in times_list:
            ...
            df_mean, df_sem = self.get_mean_and_sem_by_hour(df_)
            df_sleep = self.find_amount_of_sleep(df_)
        ...
        self.zip_dir(SCRIPT_OUTPUT, INPUT_FRONT['file_name'])